...
template <typename GridDataType> struct DataItemAccessor
{
...
template <typename GridDataType> class WeatherGridData
{
public:
...
// @param filename: needs to be under the form "prefix_latitude_suffix" where prefix and suffix can be anything but
// should not contain character '_'
...
struct HourlyTempraturesEstimator
{
	...
	//! output variables used for debug
	double tsunset = (std::numeric_limits<double>::max)();
	double prev_tsunset = (std::numeric_limits<double>::max)();
