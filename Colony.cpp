...
// When the following are all true (non-foraging day, no brood, no larvae), do not age the adults
{
	...
	if ((GlobalOptions::Get().ShouldApplyLifespanReduction()) && bWorker) // Turn on or off
	{
		double   PropRedux;
		POSITION pos = GetHeadPosition();
		int      day = 1;
...
int CAdultlist::GetQuantity()
{
	int      quan = CBeelist::GetQuantity();
	POSITION pos = PendingAdults ? PendingAdults->GetHeadPosition() : NULL;
	CAdult*  temp;
	while (pos != NULL)
	{
...
int CAdultlist::GetCabooseQuantity() const
{
	int quantity = 0;
	if (Caboose)
	{
		quantity = Caboose->GetNumber();
	}
	return quantity;
}

void CAdultlist::ClearCaboose()
{
	Caboose = NULL;
}
...
int CForageBasedAgingAdultList::GetQuantityAt(int index)
{
	// let's return the total number of bees with age index+1
	const float minAge = std::floor(index);
	const float maxAge = std::floor(index + 1);

	int      count = 0;
	POSITION pos = GetHeadPosition();
	while (pos != nullptr)
	{
		auto adult = (CAdult*)GetNext(pos);
		if (adult->GetCurrentAge() >= minAge)
		{
			if (adult->GetCurrentAge() < maxAge)
			{
				count += adult->GetNumber();
			}
			else
			{
				// bees are too old
...
int CForageBasedAgingAdultList::GetQuantityAt(int from, int to)
{
	// let's return the total number of bees with age index+1
	const float minAge = std::floor(from);
	const float maxAge = std::floor(to + 1);
...
void CForageBasedAgingAdultList::SetQuantityAt(int index, int Quan)
{
	NOT_IMPLEMENTED();
}

void CForageBasedAgingAdultList::SetQuantityAt(int from, int to, int Quan)
{
	NOT_IMPLEMENTED();
}

void CForageBasedAgingAdultList::SetQuantityAtProportional(int from, int to, double Proportion)
{
	NOT_IMPLEMENTED();
}
		...
		if ((GlobalOptions::Get().ShouldApplyLifespanReduction()) && bWorker) // Turn on or off
		{
			...
			while (pos != nullptr)
			{
				...
				// If already killed, don't kill again, if no mites, ignore
				{
					index = NumMites / theAdult->GetNumber();
					PropRedux = m_pColony->LongRedux[int(index)];
					MaxAge = (1 - PropRedux) * (theAdult->GetLifespan() + GetColony()->m_CurrentForagerLifespan);
					if (theAdult->GetCurrentAge() > MaxAge)
					{
						theAdult->Kill();
					}
				}
...
int CForageBasedAgingAdultList::MoveToEnd(int QuantityToMove, int MinAge)
{
	...
	int      CurrentlyMoving = 0;
	POSITION pos = GetTailPosition();
	auto     latestAdult = (CAdult*)GetPrev(pos);
	while (pos != nullptr && TotalMoved < QuantityToMove)
	{
		auto adult = (CAdult*)GetPrev(pos);
		if (adult->GetCurrentAge() >= (float)MinAge)
		{
			CurrentlyMoving = adult->GetNumber();
			if (CurrentlyMoving > QuantityToMove - TotalMoved) // In this case, don't move all bees in this boxcar
			{
				CurrentlyMoving = QuantityToMove - TotalMoved;
			}
			latestAdult->SetNumber(latestAdult->GetNumber() + CurrentlyMoving);
			adult->SetNumber(adult->GetNumber() - CurrentlyMoving);
			if (CurrentlyMoving > 0) TRACE("Moving Adults to End: %d\n", CurrentlyMoving);
			TotalMoved += CurrentlyMoving;
		}
	}
...
void CForageBasedAgingAdultList::UpdateCaboose(bool bWorker, CColony* colony)
{
	...
	while (pos != nullptr)
	{
		...
		if (adult->GetCurrentAge() < m_ListLength) break;
		...
		caboose->IncCurrentAge(-m_ListLength);
		if (!bWorker)
		{
			// Update stats for dead drones
...
int CForageBasedAgingAdultList::GetCabooseQuantity() const
{
	int quantity = 0;
	std::for_each(
	    m_Caboose.begin(), m_Caboose.end(), [&quantity](const CAdult* adult) { quantity += adult->GetNumber(); });
	return quantity;
}

int CForageBasedAgingAdultList::GetCabooseBoxCardsCount() const
{
	return m_Caboose.size();
}

void CForageBasedAgingAdultList::ClearCaboose()
{
	CAdult* adult = nullptr;
...
CColony CColony::operator=(CColony col)
{
	...
	// FIXME: This is not correct since we are copying pointer
	// FIXME: we should copy objects

	POSITION pos = col.Dadl.GetHeadPosition();
	while (pos != NULL)
		temp.Dadl.AddTail(col.Dadl.GetNext(pos));

	pos = col.Foragers()->GetHeadPosition();
	while (pos != NULL)
		temp.Foragers()->AddTail(col.Foragers()->GetNext(pos));

	pos = col.Wlarv.GetHeadPosition();
	while (pos != NULL)
		temp.Wlarv.AddTail(col.Wlarv.GetNext(pos));

	pos = col.Dlarv.GetHeadPosition();
	while (pos != NULL)
		temp.Dlarv.AddTail(col.Dlarv.GetNext(pos));

	pos = col.Wadl()->GetHeadPosition();
	while (pos != NULL)
		temp.Wadl()->AddTail(col.Wadl()->GetNext(pos));
...
CColony::CColony(CColony& col) // Copy Constructor
{
	...
	while (pos != NULL)
		Wadl()->AddTail(col.Wadl()->GetNext(pos));

	pos = col.Dadl.GetHeadPosition();
	while (pos != NULL)
		Dadl.AddTail(col.Dadl.GetNext(pos));

	pos = col.Foragers()->GetHeadPosition();
	while (pos != NULL)
		Foragers()->AddTail(col.Foragers()->GetNext(pos));

	pos = col.Wlarv.GetHeadPosition();
	while (pos != NULL)
		Wlarv.AddTail(col.Wlarv.GetNext(pos));

	pos = col.Dlarv.GetHeadPosition();
	while (pos != NULL)
		Dlarv.AddTail(col.Dlarv.GetNext(pos));

	pos = col.Weggs.GetHeadPosition();
	while (pos != NULL)
		Weggs.AddTail(col.Weggs.GetNext(pos));

	pos = col.Dlarv.GetHeadPosition();
	while (pos != NULL)
		Deggs.AddTail(col.Dlarv.GetNext(pos));
}
...
std::unique_ptr<CAdultlist>& CColony::Wadl()
{
	if (m_Wadl == nullptr)
	{
		if (GlobalOptions::Get().ShouldAdultsAndForagersAgeBasedOnForageInc())
		{
			m_Wadl.reset(new CForageBasedAgingAdultList);
		}
		else
		{
			m_Wadl.reset(new CAdultlist);
		}
	}
	return m_Wadl;
}

std::unique_ptr<CForagerlistA>& CColony::Foragers()
{
	if (m_Foragers == nullptr)
	{
		if (GlobalOptions::Get().ShouldAdultsAndForagersAgeBasedOnForageInc())
		{
			m_Foragers.reset(new CForageBasedAgingForagersList);
		}
		else
		{
			m_Foragers.reset(new CForagerlistA);
		}
	}
	return m_Foragers;
}
...
void CColony::Serialize(CArchive& ar, int FileFormatVersion)
{
	...
	{             // loading code
		...
		// 	double maxEggs;
		// 	ar >> maxEggs;
		// 	queen.SetMaxEggs(maxEggs);

		// 	int dayOne;
		// 	ar >> dayOne;
		// 	queen.SetDayOne(dayOne);
		// }
...
void CColony::Clear()
{
	...
	queen.SetStrength(
	    m_InitCond.m_QueenStrength); // Reset this in case requeening happened, reset to original initial condition
}
...
void CColony::InitializeBees()
{
	...
	for (i = 0; i < Foragers()->GetLength(); i++) // Distribute remaining into Foragers
	{
		...
		if (i < (Foragers()->GetLength() - 1)) theForager = new CAdult(e);
			...
			theForager = new CAdult(e + del);
		theForager->SetLifespan(Foragers()->GetLength());
		theForager->SetCurrentAge(Foragers()->GetLength() - i);
...
void CColony::UpdateBees(CEvent* pEvent, int DayNum)
{
	...
	if (theDate.ParseDateTime(dateStg)) // The date string was valid
	{
		...
		if ((m_InitCond.m_EggTransitionDRV.GetActiveValue(theDate, PropTransition)) &&
		    (m_InitCond.m_EggTransitionDRV.IsEnabled()))
		{
		...
		if ((m_InitCond.m_LarvaeTransitionDRV.GetActiveValue(theDate, PropTransition)) &&
		    (m_InitCond.m_LarvaeTransitionDRV.IsEnabled()))
		{
		...
		if ((m_InitCond.m_BroodTransitionDRV.GetActiveValue(theDate, PropTransition)) &&
		    (m_InitCond.m_BroodTransitionDRV.IsEnabled()))
		{
		...
		if ((m_InitCond.m_AdultTransitionDRV.GetActiveValue(theDate, PropTransition)) &&
		    (m_InitCond.m_AdultTransitionDRV.IsEnabled()))
		{
		...
		if ((m_InitCond.m_AdultLifespanDRV.GetActiveValue(theDate, AdultAgeLimit)) &&
		    (m_InitCond.m_AdultLifespanDRV.IsEnabled()))
		{
		...
		if ((m_InitCond.m_ForagerLifespanDRV.GetActiveValue(theDate, ForagerLifespan)) &&
		    (m_InitCond.m_ForagerLifespanDRV.IsEnabled()))
		{
...
#define DEBUG_SPECIFIC_DAY
	...
	// Geeting the number of foragers before aging process
	m_InOutEvent.m_ForagersAtTheBeginningOfTheDay = Foragers()->GetQuantity();

	// TODO:
	int NumberOfNonAdults = Wlarv.GetQuantity() + Dlarv.GetQuantity() + CapDrn.GetQuantity() + CapWkr.GetQuantity() +
	                        CapWkr.GetCaboose()->GetNumber() + CapDrn.GetCaboose()->GetNumber();
	...
	const bool ForageIncIsValid =
	    GlobalOptions::Get().ShouldForageDayElectionBasedOnTemperatures() || pEvent->GetForageInc() > 0.0;

	// TODO:
	if ((NumberOfNonAdults > 0) || (pEvent->IsForageDay() && ForageIncIsValid))
	{

		// Foragers killed due to pesticide.  Recruit precocious Adult Workers to be foragers - Add them to the last
		// Adult Boxcar The last boxcar will be moved to the Caboose when Wadl()->Update is called a little later.  The
		// Wadl Caboose will be moved to the forager list when forager.Update is called.
		//
		int ForagersToBeKilled = QuantityPesticideToKill(
		    Foragers().get(), m_EPAData.m_D_C_Foragers, 0, m_EPAData.m_AI_AdultLD50_Contact,
		    m_EPAData.m_AI_AdultSlope_Contact); // Contact Mortality
		ForagersToBeKilled += QuantityPesticideToKill(
		    Foragers().get(), m_EPAData.m_D_D_Foragers, 0, m_EPAData.m_AI_AdultLD50,
		    m_EPAData.m_AI_AdultSlope); // Diet Mortality
		...
		auto forageBasedAdultList = dynamic_cast<CForageBasedAgingAdultList*>(Wadl().get());
		if (forageBasedAdultList == nullptr)
		{
			...
			if (agingAdults)
			{
				...
				m_InOutEvent.m_WAdultToForagers = Wadl()->GetCaboose() ? Wadl()->GetCaboose()->GetNumber() : 0;
		...
		{
			auto forageBasedForagersList = dynamic_cast<CForageBasedAgingForagersList*>(Foragers().get());

			ASSERT(forageBasedForagersList != nullptr);

			Dadl.Update((CBrood*)CapDrn.GetCaboose(), this, pEvent, false);
			forageBasedAdultList->Update((CBrood*)CapWkr.GetCaboose(), this, pEvent, true);

			// Update stats for adults becoming foragers
			m_InOutEvent.m_WAdultToForagers = forageBasedAdultList->GetCabooseQuantity();

			forageBasedForagersList->Update(forageBasedAdultList->GetCabooseQueue(), this, pEvent);
		}
...
void CColony::AddMites(CMite NewMites)
{
	...
	if (RunMite.GetTotal() <= 0) PropRMVirgins = 1.0;
		...
		PropRMVirgins = virgins.GetTotal() / RunMite.GetTotal();
}
...
void CColony::InitializeMites()
{
	...
	RunMiteW = CMite(
	    0, int((Wadl()->GetQuantity() * m_InitCond.m_workerAdultInfestField +
	            Foragers()->GetQuantity() * m_InitCond.m_workerAdultInfestField) /
	           float(100)));
...
#define MAXMITES_PER_WORKER_CELL 4
	...
	// Modify potential Drone Mite infestation rate by factoring in liklihood of finding a drone target cell in all the
	// cells
	...
	*/
	...
	if (WkrEmerge.GetNumber() == 0) MitesPerCellW = 0;
		...
		MitesPerCellW = (double)WkrEmerge.m_Mites.GetTotal() / WkrEmerge.GetNumber();
	if (DrnEmerge.GetNumber() == 0) MitesPerCellD = 0;
	else
		MitesPerCellD = (double)DrnEmerge.m_Mites.GetTotal() / DrnEmerge.GetNumber();
	...
	if (MitesPerCellW <= 1.0) ReproMitePerCellW = m_InitCond.m_workerMiteOffspring;
	else
		ReproMitePerCellW = double((1.15 * MitesPerCellW) - (0.233 * MitesPerCellW * MitesPerCellW));
	if (ReproMitePerCellW < 0) ReproMitePerCellW = 0;

	if (MitesPerCellD <= 1.0) ReproMitePerCellD = m_InitCond.m_droneMiteOffspringField;
	else
		ReproMitePerCellD = double(1.11 + (0.151 * MitesPerCellD) - (0.3 * MitesPerCellD * MitesPerCellD));
...
#define PROPRUNMITE2 0.6
	...
	RunMite = RunMite + NewMitesW + NewMitesD + (SurviveMitesW + SurviveMitesD) * PROPRUNMITE2;
	if (RunMite.GetTotal() <= 0) PropRMVirgins = 1.0;
	else
		PropRMVirgins = double(RunMiteVirgins.GetTotal() + NewMitesW.GetTotal() + NewMitesD.GetTotal()) /
		                double(RunMite.GetTotal());
	...
	if (m_VTEnable)
	{
		...
		if (m_MiteTreatmentInfo.GetActiveItem(*theDate, theItem))
		{
			...
			RunMite.SetNonResistant(int(double(RunMite.GetNonResistant()) * (100.0 - theItem.PctMortality) / 100.0));
...
void CColony::ReQueenIfNeeded(
    int SimDayNum, CEvent* theEvent, UINT EggLayingDelay, double WkrDrnRatio, BOOL EnableReQueen,
	...
	*/
	...
	if (Scheduled == 0)
	{
		if ( // Initial re-queening
		    ...
		    ((ReQueenDate.GetYear() < theEvent->GetTime().GetYear()) &&
		     (ReQueenDate.GetMonth() == theEvent->GetTime().GetMonth()) &&
		     (ReQueenDate.GetDay() == theEvent->GetTime().GetDay()) && (RQOnce != 0)))
		{
	...
	{
		if ((queen.GetPropDroneEggs() > 0.15) && (theEvent->GetTime().GetMonth() > 3) &&
...
{
	// Worker Larvae 4
	// if (m_EPAData.m_D_L4 > m_EPAData.m_D_L4_Max) // IED - only reduce if current dose greater than previous maximum
	// dose
	{
		m_DeadWorkerLarvaePesticide = ApplyPesticideToBees(
		    &Wlarv, 3, 3, m_EPAData.m_D_L4, 0, m_EPAData.m_AI_LarvaLD50, m_EPAData.m_AI_LarvaSlope);
	...
	{
		m_DeadWorkerLarvaePesticide += ApplyPesticideToBees(
		    &Wlarv, 4, 4, m_EPAData.m_D_L5, m_EPAData.m_D_L5_Max, m_EPAData.m_AI_LarvaLD50, m_EPAData.m_AI_LarvaSlope);
		m_EPAData.m_D_L5_Max = m_EPAData.m_D_L5;
	}

	// Drone Larvae
	m_DeadDroneLarvaePesticide = ApplyPesticideToBees(
	    &Dlarv, 3, 3, m_EPAData.m_D_LD, 0, m_EPAData.m_AI_LarvaLD50,
	    m_EPAData.m_AI_LarvaSlope); // New L4 drones
	if (m_EPAData.m_D_LD > m_EPAData.m_D_LD_Max)
	{
		m_DeadDroneLarvaePesticide += ApplyPesticideToBees(
		    &Dlarv, 4, DLARVLIFE - 1, m_EPAData.m_D_LD, m_EPAData.m_D_LD_Max, m_EPAData.m_AI_LarvaLD50,
		    m_EPAData.m_AI_LarvaSlope);
		m_EPAData.m_D_LD_Max = m_EPAData.m_D_LD;
	}

	// Worker Adults 1-3
	m_DeadWorkerAdultsPesticide = ApplyPesticideToBees(
	    Wadl().get(), 0, 0, m_EPAData.m_D_A13, 0, m_EPAData.m_AI_AdultLD50,
	    m_EPAData.m_AI_AdultSlope); // New adults
	if (m_EPAData.m_D_A13 > m_EPAData.m_D_A13_Max)
	{
		m_DeadWorkerAdultsPesticide += ApplyPesticideToBees(
		    Wadl().get(), 1, 2, m_EPAData.m_D_A13, m_EPAData.m_D_A13_Max, m_EPAData.m_AI_AdultLD50,
		    m_EPAData.m_AI_AdultSlope);
	...
	{
		m_DeadWorkerAdultsPesticide += ApplyPesticideToBees(
		    Wadl().get(), 3, 9, m_EPAData.m_D_A410, m_EPAData.m_D_A410_Max, m_EPAData.m_AI_AdultLD50,
		    m_EPAData.m_AI_AdultSlope);
	...
	{
		m_DeadWorkerAdultsPesticide += ApplyPesticideToBees(
		    Wadl().get(), 10, WADLLIFE - 1, m_EPAData.m_D_A1120, m_EPAData.m_D_A1120_Max, m_EPAData.m_AI_AdultLD50,
		    m_EPAData.m_AI_AdultSlope);
		m_EPAData.m_D_A1120_Max = m_EPAData.m_D_A1120;
	}

	// Worker Drones
	m_DeadDroneAdultsPesticide =
	    ApplyPesticideToBees(&Dadl, 0, 0, m_EPAData.m_D_AD, 0, m_EPAData.m_AI_AdultLD50, m_EPAData.m_AI_AdultSlope);
	if (m_EPAData.m_D_AD > m_EPAData.m_D_AD_Max)
	{
		m_DeadDroneAdultsPesticide += ApplyPesticideToBees(
		    &Dadl, 1, DADLLIFE - 1, m_EPAData.m_D_AD, m_EPAData.m_D_AD_Max, m_EPAData.m_AI_AdultLD50,
		    m_EPAData.m_AI_AdultSlope);
		m_EPAData.m_D_AD_Max = m_EPAData.m_D_AD;
	}

	// Foragers - Contact Mortality
	m_DeadForagersPesticide = ApplyPesticideToBees(
	    Foragers().get(), 0, 0, m_EPAData.m_D_C_Foragers, 0, m_EPAData.m_AI_AdultLD50_Contact,
	    m_EPAData.m_AI_AdultSlope_Contact);
	if (m_EPAData.m_D_C_Foragers > m_EPAData.m_D_C_Foragers_Max)
	{
		m_DeadForagersPesticide += ApplyPesticideToBees(
		    Foragers().get(), 1, Foragers()->GetLength() - 1, m_EPAData.m_D_C_Foragers, m_EPAData.m_D_C_Foragers_Max,
		    m_EPAData.m_AI_AdultLD50_Contact, m_EPAData.m_AI_AdultSlope_Contact);
		m_EPAData.m_D_C_Foragers_Max = m_EPAData.m_D_C_Foragers;
	}

	// Foragers - Diet Mortality
	m_DeadForagersPesticide += ApplyPesticideToBees(
	    Foragers().get(), 0, 0, m_EPAData.m_D_D_Foragers, 0, m_EPAData.m_AI_AdultLD50, m_EPAData.m_AI_AdultSlope);
	if (m_EPAData.m_D_D_Foragers > m_EPAData.m_D_D_Foragers_Max)
	{
		m_DeadForagersPesticide += ApplyPesticideToBees(
		    Foragers().get(), 1, Foragers()->GetLength() - 1, m_EPAData.m_D_D_Foragers, m_EPAData.m_D_D_Foragers_Max,
		    m_EPAData.m_AI_AdultLD50, m_EPAData.m_AI_AdultSlope);
...
// This calculates the number of bees to kill then reduces that number from all age groups between "from" and "to" in
// the list.
int CColony::ApplyPesticideToBees(
...
void CColony::DetermineFoliarDose(int DayNum)
{
	...
	if ((CurDate >= m_EPAData.m_FoliarAppDate) && (CurDate >= m_EPAData.m_FoliarForageBegin) &&
	    (CurDate < m_EPAData.m_FoliarForageEnd))
	{
...
void CColony::ConsumeFood(CEvent* pEvent, int DayNum)
{
	...
	if (pEvent->IsForageDay())
	{
		...
		if (IsPollenFeedingDay(pEvent))
		{
			...
			NeedP = 0; // All pollen needs are met by paddies and all incoming pollen will be stored - no incoming
			           // needed by colony
			C_AI_P = 0;
		}
		else
		...
		if (IsNectarFeedingDay(pEvent))
		{
			...
			theResource.m_ResourseQuantity =
			    m_SuppNectar.m_StartingAmount / ((m_SuppNectar.m_EndDate - m_SuppNectar.m_BeginDate).GetDays());
	...
	m_EPAData.m_D_A410 =
	    C_Actual_P * m_EPAData.m_C_A410_Pollen / 1000.0 + C_Actual_N * m_EPAData.m_C_A410_Nectar / 1000.0;
	m_EPAData.m_D_A1120 =
	    C_Actual_P * m_EPAData.m_C_A1120_Pollen / 1000.0 + C_Actual_N * m_EPAData.m_C_A1120_Nectar / 1000.0;
	m_EPAData.m_D_AD = C_Actual_P * m_EPAData.m_C_AD_Pollen / 1000.0 + C_Actual_N * m_EPAData.m_C_AD_Nectar / 1000.0;
	m_EPAData.m_D_D_Foragers =
	    C_Actual_P * m_EPAData.m_C_Forager_Pollen / 1000.0 + C_Actual_N * m_EPAData.m_C_Forager_Nectar / 1000.0;
}
...
bool CColony::IsPollenFeedingDay(CEvent* pEvent)
{
	...
	if ((m_SuppPollenEnabled) && (GetColonySize() > 100)) // Supplemental enabled and colony still alive
	{
		...
		{
			COleDateTime TestBeginTime(
			    pEvent->GetTime().GetYear(), m_SuppPollen.m_BeginDate.GetMonth(), m_SuppPollen.m_BeginDate.GetDay(), 0,
			    0, 0);
			COleDateTime TestEndTime(
			    pEvent->GetTime().GetYear(), m_SuppPollen.m_EndDate.GetMonth(), m_SuppPollen.m_EndDate.GetDay(), 0, 0,
			    0);

			FeedingDay =
			    ((m_SuppPollen.m_CurrentAmount > 0.0) && (TestBeginTime < pEvent->GetTime()) &&
			     (TestEndTime >= pEvent->GetTime()));
		}
		else
		{
			FeedingDay =
			    ((m_SuppPollen.m_CurrentAmount > 0.0) && (m_SuppPollen.m_BeginDate < pEvent->GetTime()) &&
...
bool CColony::IsNectarFeedingDay(CEvent* pEvent)
{
	...
	if ((m_SuppNectarEnabled) && (GetColonySize() > 100)) // Supplemental enabled and colony still alive
	{
		...
		{
			COleDateTime TestBeginTime(
			    pEvent->GetTime().GetYear(), m_SuppNectar.m_BeginDate.GetMonth(), m_SuppNectar.m_BeginDate.GetDay(), 0,
			    0, 0);
			COleDateTime TestEndTime(
			    pEvent->GetTime().GetYear(), m_SuppNectar.m_EndDate.GetMonth(), m_SuppNectar.m_EndDate.GetDay(), 0, 0,
			    0);

			FeedingDay =
			    ((m_SuppNectar.m_CurrentAmount > 0.0) && (TestBeginTime < pEvent->GetTime()) &&
			     (TestEndTime >= pEvent->GetTime()));
		}
		else
		{
			FeedingDay =
			    ((m_SuppNectar.m_CurrentAmount > 0.0) && (m_SuppNectar.m_BeginDate < pEvent->GetTime()) &&
...
double CColony::GetPollenNeeds(CEvent* pEvent)
{
	...
	if (pEvent->IsWinterDay())
	{
		...
		for (int i = 0; i < 3; i++)
		{
			if (WadlAG[i] <=
			    NurseBeeQuantity -
			        MovedNurseBees) // This age group has fewer bees than required to fill nurse bee quantity
			{
		...
		if (pEvent->IsForageDay())
		{
			...
			ForagerNeed += (Foragers()->GetQuantity() - Foragers()->GetActiveQuantity()) * m_EPAData.m_C_A1120_Pollen /
			               1000; // Unemployed consume like adults
		}
		else
		{
	...
	{
		// Larvae needs
		double LNeeds = Wlarv.GetQuantityAt(3) * m_EPAData.m_C_L4_Pollen +
		                Wlarv.GetQuantityAt(4) * m_EPAData.m_C_L5_Pollen +
		...
		if (pEvent->IsForageDay())
		{
			ANeeds = Wadl()->GetQuantityAt(0, 2) * m_EPAData.m_C_A13_Pollen +
			         ...
			         Wadl()->GetQuantityAt(10, 19) * m_EPAData.m_C_A1120_Pollen +
			         Dadl.GetQuantity() * m_EPAData.m_C_AD_Pollen +
			         Foragers()->GetActiveQuantity() * m_EPAData.m_C_Forager_Pollen +
			         Foragers()->GetUnemployedQuantity() * m_EPAData.m_C_A1120_Pollen;
		...
		else // All foragers consume the same quantity as the most mature adult house bees on non-forage days
		{
			ANeeds = Wadl()->GetQuantityAt(0, 2) * m_EPAData.m_C_A13_Pollen +
			         Wadl()->GetQuantityAt(3, 9) * m_EPAData.m_C_A410_Pollen +
			         (Wadl()->GetQuantityAt(10, 19) + Foragers()->GetQuantity()) * m_EPAData.m_C_A1120_Pollen +
			         Dadl.GetQuantity() * m_EPAData.m_C_AD_Pollen;
		}
...
double CColony::GetNectarNeeds(CEvent* pEvent)
{
			...
			else //  8.5 < AveTemp < 18.0
			{
				if (pEvent->IsForageDay()) // In this case, foragers need normal forager nutrition
				{
					...
					Need = (Foragers()->GetActiveQuantity() * m_EPAData.m_C_Forager_Nectar) / 1000 +
					       0.05419 * NonForagers * pow(0.128 * NonForagers, -0.27); // grams
				}
	...
	else // Summer Day
	{
		...
		// Larvae needs
		double LNeeds = Wlarv.GetQuantityAt(3) * m_EPAData.m_C_L4_Nectar +
		                Wlarv.GetQuantityAt(4) * m_EPAData.m_C_L5_Nectar +
		...
		if (pEvent->IsForageDay())
		{
			ANeeds = Wadl()->GetQuantityAt(0, 2) * m_EPAData.m_C_A13_Nectar +
			         ...
			         Wadl()->GetQuantityAt(10, 19) * m_EPAData.m_C_A1120_Nectar +
			         Foragers()->GetUnemployedQuantity() * m_EPAData.m_C_A1120_Nectar +
			         Foragers()->GetActiveQuantity() * m_EPAData.m_C_Forager_Nectar +
			         Dadl.GetQuantity() * m_EPAData.m_C_AD_Nectar;
		}
		else // Foragers consume the same quantity as the most mature adult house bees
		{
			ANeeds = Wadl()->GetQuantityAt(0, 2) * m_EPAData.m_C_A13_Nectar +
			         Wadl()->GetQuantityAt(3, 9) * m_EPAData.m_C_A410_Nectar +
			         (Wadl()->GetQuantityAt(10, 19) + Foragers()->GetQuantity()) * m_EPAData.m_C_A1120_Nectar +
			         Dadl.GetQuantity() * m_EPAData.m_C_AD_Nectar;
		}
...
double CColony::GetIncomingNectarPesticideConcentration(int DayNum)
{
	...
	{
		if ((m_EPAData.m_FoliarEnabled) && (CurDate >= m_EPAData.m_FoliarAppDate) &&
		    (CurDate >= m_EPAData.m_FoliarForageBegin) && (CurDate < m_EPAData.m_FoliarForageEnd))
		{
			IncomingConcentration =
			    110.0 * m_EPAData.m_E_AppRate / 1000000.0; // Incoming AI Grams/Grams Nectar due to foliar spray
			// Dose reduced due to active ingredient half-life
			LONG DaysSinceApplication = (CurDate - m_EPAData.m_FoliarAppDate).GetDays();
			if (m_EPAData.m_AI_HalfLife > 0)
			{
		...
		if ((CurDate >= m_EPAData.m_SeedForageBegin) && (CurDate < m_EPAData.m_SeedForageEnd) &&
		    (m_EPAData.m_SeedEnabled))
		{
			IncomingConcentration += m_EPAData.m_E_SeedConcentration /
			                         1000000.0; // Add Incoming Grams AI/Grams Bectar due to seed concentration
			// TRACE("Incoming Seed Nectar Pesticide on %s\n",CurDate.Format("%m/%d/%Y"));
			AddEventNotification(CurDate.Format("%m/%d/%Y"), "Incoming Seed Nectar Pesticide");
		}
		if ((CurDate >= m_EPAData.m_SoilForageBegin) && (CurDate < m_EPAData.m_SoilForageEnd) &&
		    (m_EPAData.m_SoilEnabled)) // In soil forage range
		{
			if ((m_EPAData.m_AI_KOW > 0) || (m_EPAData.m_E_SoilTheta != 0)) // Ensure no NAN exceptions
			{
				...
				double SoilConc =
				    TSCF * (pow(10, (0.95 * LogKOW - 2.05)) + 0.82) * m_EPAData.m_E_SoilConcentration *
				    (m_EPAData.m_E_SoilP /
				     (m_EPAData.m_E_SoilTheta + m_EPAData.m_E_SoilP * m_EPAData.m_AI_KOC * m_EPAData.m_E_SoilFoc));
				//					m_EPAData.m_E_SoilConcentration*((m_EPAData.m_E_SoilP/m_EPAData.m_E_SoilTheta) +
				// m_EPAData.m_E_SoilP*m_EPAData.m_AI_KOC*m_EPAData.m_E_SoilFoc);
				IncomingConcentration +=
				    SoilConc / 1000000; // Add Grams AI/Gram Nectar for soil concentration
...
double CColony::GetIncomingPollenPesticideConcentration(int DayNum)
{
	...
	else // Normal process
	{
		...
		{
			IncomingConcentration =
			    110.0 * m_EPAData.m_E_AppRate /
			    1000000; // Incoming AI Grams/Grams Pollen due to foliar spray (uG/G = 110  * lbs/AC)
			// Dose reduced due to active ingredient half-life
			LONG DaysSinceApplication = (CurDate - m_EPAData.m_FoliarAppDate).GetDays();
			if (m_EPAData.m_AI_HalfLife > 0)
			{
