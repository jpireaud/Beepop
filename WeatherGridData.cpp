...
template <> struct DataItemAccessor<ObservedHistoricalItem>
{
...
template <typename GridDataType> WeatherGridData<GridDataType>::WeatherGridData()
{
}
...
template <typename GridDataType> void WeatherGridData<GridDataType>::load(const std::string& filename)
{
	m_data.clear();
...
template <> COleDateTime WeatherGridData<ObservedHistoricalItem>::getStartTime() const
{
	return COleDateTime(1979, 1, 1, 0, 0, 0);
}

template <> COleDateTime WeatherGridData<ModeledHistoricalItem>::getStartTime() const
{
	return COleDateTime(1950, 1, 1, 0, 0, 0);
}

template <> COleDateTime WeatherGridData<Rcp85>::getStartTime() const
{
	return COleDateTime(2006, 1, 1, 0, 0, 0);
}

template <typename GridDataType> COleDateTime WeatherGridData<GridDataType>::getEndTime() const
{
...
{
template <typename GridDataType> WeatherGridData<GridDataType> LoadGridData(const std::string& filename)
{
...
DayLengthResult DayLength(float latitude, int jDay)
{
	...
	const double delta =
	    180.0 / M_PI *
	    (0.006918 - 0.399912 * std::cos(gamma) + 0.070257 * std::sin(gamma) - 0.006758 * std::cos(gamma) +
	     0.000907 * std::sin(gamma) - 0.002697 * std::cos(3.0 * (gamma)) + 0.00148 * std::sin(3.0 * (gamma)));
	const double cosWo =
	    (sin(-0.8333 / 360.0 * 2.0 * M_PI) - sin(latitude / 360.0 * 2.0 * M_PI) * sin(delta / 360.0 * 2.0 * M_PI)) /
	    (cos(latitude / 360.0 * 2.0 * M_PI) * cos(delta / 360.0 * 2.0 * M_PI));
...
double DayLightHours(float latitude, int JDay)
{
	...
	double D = 24 - (24 / M_PI) * std::acos(
	                                  (std::sin(0.833 * M_PI / 180) + (std::sin(latitude * M_PI / 180) * std::sin(P))) /
	                                  (std::cos(latitude * M_PI / 180) * std::cos(P)));
	return D;
}
...
{
	tsunset = tmin + ((tmax)-tmin) * std::sin(M_PI * (daylength / ((daylength) + 4)));
	...
	for (int h = 0; h < hourly_temperatures.size(); h++)
	{
		if (h <= sunrise)
		{
			if (prev_tmin != (std::numeric_limits<double>::max)())
			{
				hourly_temperatures[h] =
				    prev_tsunset - ((prev_tsunset - tmin) / std::log((std::max)(1.0, 24.0 - ((prev_sunset)-sunrise))) *
				                    std::log(h + 24.0 - prev_sunset + 1));
			}
			else
			{
		...
		else if (h > sunset)
		{
			if (next_tmin != (std::numeric_limits<double>::max)())
			{
				hourly_temperatures[h] =
				    tsunset -
				    ((tsunset - next_tmin) / std::log(24.0 - ((sunset)-next_sunrise) + 1) * std::log(h - sunset + 1));
			}
			else
			{
				hourly_temperatures[h] = tmin;
...
int HourlyTempraturesEstimator::count_dayligth(
    double temperatureMinThreshold /*= 12.0*/, double temperatureMaxThreshold /*= 43.33*/)
{
	int count = 0;
