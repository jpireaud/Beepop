...
public:
	...
	virtual std::string describe() const override
	{
		...
		ss << "is matching sunrise:" << m_result.sunrise << " sunset:" << m_result.sunset
		   << " daylength:" << m_result.daylength;
		return ss.str();
	}
};
...
template <typename Container, typename Compare> struct CompareMatcher : Catch::MatcherBase<Container>
{

	CompareMatcher(const Container& comparator, const Compare& compare) : m_comparator(comparator), m_compare(compare)
	{
	}
	...
	virtual std::string describe() const override { return "Equals: " + Catch::Detail::stringify(m_comparator); }
...
TEST_CASE("Temperature Data", "[input]")
{
	...
	SECTION("Load Rcp85Binary - Check last day data")
	{
		...
		bd::date startDate(2006, bd::Jan, 1);
		bd::date endDate = startDate + bd::days(data.data().size() - 1 /* first day*/);
		std::cout << std::setw(10) << "ppt" << std::setw(10) << "max" << std::setw(10) << "min" << std::setw(10)
		          << "wind" << std::endl;
		for (auto it = data.data().end() - 6; it != data.data().end(); it++)
		{
			auto accessor = DataItemAccessor(*it);
			std::cout << std::setw(10) << accessor.PPT() << std::setw(10) << accessor.TMAX() << std::setw(10)
			          << accessor.TMIN() << std::setw(10) << accessor.WIND() << std::endl;
		}
		CHECK(endDate.year() == 2099);
		CHECK(endDate.month() == 12);
		CHECK(endDate.day() == 31);
	...
	SECTION("hourly temperature")
	{
		// JDay 1
		{
			...
			std::array<std::float_t, 24> expectedResult = {
			    -2.550000f,  -2.550000f, -2.550000f, -2.550000f,  -2.550000f,  -2.550000f,  -2.550000f, -2.550000f,
			    -2.377236f,  -1.824559f, -1.316665f, -0.8849064f, -0.5559374f, -0.3500657f, -0.28f,     -0.3500657f,
			    -0.5559374f, -1.787940f, -2.794280f, -3.479060f,  -3.998755f,  -4.417698f,  -4.768682f, -5.070708f};
...
TEST_CASE("Temperature Data Failing", "[hide]")
{
	SECTION("Hourly Temperature")
	{
		// JDay 2
		{
			...
			std::array<std::float_t, 24> expectedResult = {
			    -5.444190f, -5.685756f, -5.903585f, -6.101928f,  -6.283987f, -6.452233f, -6.608616f, -6.754699f,
			    -6.074356f, -4.088088f, -2.263309f, -0.7124124f, 0.4690795f, 1.2083956f, 1.46f,      1.2083956f,
			    0.4690795f, -1.082260f, -2.173649f, -2.915478f,  -3.478154f, -3.931593f, -4.311390f, -4.638157f};
			CHECK_THAT(input.hourly_temperatures, EqualsApprox(expectedResult));
		}
		// JDay 148
		{
			...
			std::array<std::float_t, 24> expectedResult = {
			    10.779638f, 10.463412f, 10.194304f, 9.960077f, 9.752718f, 9.976793f, 11.420404f, 12.79732f,
			    14.07209f,  15.21189f,  16.18737f,  16.97343f, 17.54982f, 17.90169f, 18.02f,     17.90169f,
			    17.54982f,  16.97343f,  16.18737f,  15.21189f, 13.56064f, 11.23618f, 9.791873f,  8.741350f};
			CHECK_THAT(input.hourly_temperatures, EqualsApprox(expectedResult));
		}
		// JDay last
		{
			...
			std::array<std::float_t, 24> expectedResult = {
			    9.918212f, 9.548716f, 9.234689f, 8.961630f, 8.720073f, 8.445311f, 9.399063f, 10.30797f,
			    11.14885f, 11.90026f, 12.54306f, 13.06085f, 13.44042f, 13.67211f, 13.75f,    13.67211f,
			    13.44042f, 13.06085f, 12.54306f, 11.90026f, 7.64000f,  7.64000f,  7.640000f, 7.640000f};
			CHECK_THAT(input.hourly_temperatures, EqualsApprox(expectedResult));
		}
	}
	...
	SECTION("Hourly temperature from file")
	{
		...
		for (size_t dayIndex = startIndex; dayIndex < endIndex; dayIndex++)
		{
			...
			std::array<float, 24> expectedResult = {
			    expectedItem.hour_0,  expectedItem.hour_1,  expectedItem.hour_2,  expectedItem.hour_3,
			    expectedItem.hour_4,  expectedItem.hour_5,  expectedItem.hour_6,  expectedItem.hour_7,
			    expectedItem.hour_8,  expectedItem.hour_9,  expectedItem.hour_10, expectedItem.hour_11,
			    expectedItem.hour_12, expectedItem.hour_13, expectedItem.hour_14, expectedItem.hour_15,
			    expectedItem.hour_16, expectedItem.hour_17, expectedItem.hour_18, expectedItem.hour_19,
