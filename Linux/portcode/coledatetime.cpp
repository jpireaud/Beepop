...
COleDateTime::COleDateTime() : m_date(1899, 12, 30, 0, 0, 0)
{
	m_status = valid;
}

COleDateTime::COleDateTime(const Poco::DateTime& date) : m_date(date)
{
	m_status = valid;
}

COleDateTime::COleDateTime(int32_t nYear, int32_t nMonth, int32_t nDay, int32_t nHour, int32_t nMin, int32_t nSec)
{
	try
	{
...
bool COleDateTime::ParseDateTime(const CString& dateTimeStr, DWORD dwFlags)
{
	try
	{
		...
		else
		{
			...
			for (auto formatIt = supportedDateTimeFormatsLengths.begin();
			     !success && formatIt != supportedDateTimeFormatsLengths.end(); formatIt++)
			{
				if (formatIt->first == dateTimeStr.GetLength())
				{
					success = Poco::DateTimeParser::tryParse(
					    formatIt->second, dateTimeStr.ToString(), m_date, timeZoneDifferential);
				}
			}
		}
		// debug
		// std::cout << "parsing: " << dateTimeStr << " resulted in " << Poco::DateTimeFormatter::format(m_date,
		// "%m/%d/%Y %H:%M:%S") << " with success set to " << std::boolalpha << success << std::endl;
...
bool COleDateTime::GetAsDATE(DATE& date) const
{
	...
	double           daySpan =
	    (timespan.m_span.hours() + timespan.m_span.minutes() / 60.0 + timespan.m_span.seconds() / 3600.0) / 24.0;
	date = days + daySpan;
	// debug
	// std::cout << "as DATE with days " << timespan.m_span.days() << " hours " << timespan.m_span.hours() << " minutes
	// " << timespan.m_span.minutes() << " seconds " << timespan.m_span.seconds() << std::endl; std::cout << "as DATE
	// result " << date << " days " << days << " daySpan " << daySpan << std::endl;
	if (date < 0.0)
	{
...
COleDateTimeSpan::COleDateTimeSpan() : m_span()
{
}

COleDateTimeSpan::COleDateTimeSpan(const Poco::Timespan& span) : m_span(span)
{
}
...
COleDateTimeSpan::COleDateTimeSpan(double dblSpanSrc)
{
	...
	// debug
	// std::cout << dblSpanSrc << " in secs " << dblSeconds << " seconds: " << seconds << " micro: " << microseconds <<
	// std::endl;
	m_span.assign(long(seconds), long(microseconds));
}

COleDateTimeSpan::COleDateTimeSpan(size_t lDays, int32_t nHours, int32_t nMins, int32_t nSecs)
{
