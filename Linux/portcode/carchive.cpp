...
CArchive::CArchive(CFile* pFile, UINT nMode) : mStream(pFile->GetStream()), mModeMask(nMode)
{
}
...
template <> void CArchive::operator<<(const COleDateTime& date)
{
...
template <> void CArchive::operator<<(const CString& str)
{
...
template <> void CArchive::operator<<(const CTime& time)
{
	std::int64_t timeInt64 = time.GetAsTimeT();
	mStream.write((const char*)&timeInt64, sizeof(std::int64_t));
}

template <> void CArchive::operator>>(COleDateTime& date)
{
	if (mStream.rdstate() != mStream.goodbit) throw std::runtime_error("can't read from input stream");
...
template <> void CArchive::operator>>(CString& str)
{
	if (mStream.rdstate() != mStream.goodbit) throw std::runtime_error("can't read from input stream");
...
template <> void CArchive::operator>>(CTime& time)
{
	if (mStream.rdstate() != mStream.goodbit) throw std::runtime_error("can't read from input stream");
...
DWORD_PTR CArchive::ReadCount()
{
	...
	*this >> dwCount;
	...
	return dwCount;
#else  // _WIN64
	if (dwCount != 0xFFFFFFFF) return dwCount;
...
{
	if (dwCount < 0xFFFF) *this << (WORD)dwCount; // 16-bit count
	else
	{
		*this << (WORD)0xFFFF;
#ifndef _WIN64
...
#else                            // _WIN64
		if (dwCount < 0xFFFFFFFF) *this << (DWORD)dwCount; // 32-bit count
		else
		{
