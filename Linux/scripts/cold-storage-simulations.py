...
def run_simulation(command_str):
    ...
    utilities.safe_print('\tCommand:' + command_str + ' \n\t\tElapsed:' +
                         '%.2f seconds' % elapsed_time.total_seconds())
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--exe', type=str, help='Path of the VarroaPop command line application', required=True)
    parser.add_argument(
        '--vrp', type=str, help='Path of the vrp file to use for simulations', required=True)
    ...
    if not os.path.isdir(arguments.snapshots_directory):
        print('Cannot find snapshots directory at: ' +
              arguments.snapshots_directory)
        exit(-1)
    ...
    default_command = arguments.exe + ' -f -v ' + to_normalize_path(arguments.vrp) + \
        ' --climateChangeScenario  --inOutEvents'
    ...
    for weather_file in weather_files:
        ...
        location_output_directory = os.path.join(
            arguments.output_directory, info.location)
        output_directory = os.path.join(
            location_output_directory, info.scenario)

        # get input filename and check if it exists
        input_file = os.path.join(
            arguments.input_directory, info.scenario + '.txt')
        ...
        location_input_file = os.path.join(
            location_output_directory, info.scenario + '.txt')
        ...
        #              'CRResetDate=01/01/' +
        #              snapshots_init_year[info.scenario]+'\n',
        ...
        if os.path.exists(location_input_file):
            command = default_command + ' -i ' + \
                to_normalize_path(location_input_file)
        else:
            command = default_command + ' -i ' + to_normalize_path(input_file)

        command += ' -w ' + \
            to_normalize_path(os.path.join(
                arguments.weather_directory, weather_file))

        command += ' --binaryWeatherFileFormat ' + \
            utilities.get_valid_binary_format_identifier(info.scenario)
        ...
        if not arguments.skip_default:
            ...
            output_file = os.path.join(
                output_directory, output_filename + '.csv')
        ...
        for start_date in start_dates:
            for end_date in end_dates:
                output_filename = info.model + '_cold_storage_' + \
                    start_date.hyphen() + '_' + end_date.hyphen()
                output_file = os.path.join(
                    output_directory, output_filename + '.csv')
                exec_command = command + ' -o ' + \
                    to_normalize_path(output_file)
    ...
    print('Total duration             (s):' + '%.2f' %
          (datetime.datetime.now() - simulation_time).total_seconds())
